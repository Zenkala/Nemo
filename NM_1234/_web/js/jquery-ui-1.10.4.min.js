...
return parseFloat(alignValue.toFixed(5));},_valueMin:function(){return this.options.min;},_valueMax:function(){return this.options.max;},_refreshValue:function(){var lastValPercent,valPercent,value,valueMin,valueMax,oRange=this.options.range,o=this.options,that=this,animate=(!this._animateOff)?o.animate:false,_set={};if(this.options.values&&this.options.values.length){this.handles.each(function(i){valPercent=(that.values(i)-that._valueMin())/(that._valueMax()-that._valueMin())*100;_set[that.orientation==="horizontal"?"left":"bottom"]=valPercent+"%";$(this).stop(1,1)[animate?"animate":"css"](_set,o.animate);if(that.options.range===true){if(that.orientation==="horizontal"){if(i===0){if(animate){that.range.stop(1,1)["animate"]({left:valPercent+"%"},o.animate);}else{that.range.stop(1,1).css({left:valPercent+"%"},o.animate);}}
if(i===1){if(animate){that.range["animate"]({width:(valPercent-lastValPercent)+"%"},{queue:false,duration:o.animate});}else{that.range.css({width:(valPercent-lastValPercent)+"%"},{queue:false,duration:o.animate});}}}else{if(i===0){if(animate){that.range.stop(1,1)["animate"]({bottom:(valPercent)+"%"},o.animate);}else{that.range.stop(1,1).css({bottom:(valPercent)+"%"},o.animate);}}
if(i===1){if(animate){that.range["animate"]({height:(valPercent-lastValPercent)+"%"},{queue:false,duration:o.animate});}else{that.range.css({height:(valPercent-lastValPercent)+"%"},{queue:false,duration:o.animate});}}}}
lastValPercent=valPercent;});}else{value=this.value();valueMin=this._valueMin();valueMax=this._valueMax();valPercent=(valueMax!==valueMin)?(value-valueMin)/(valueMax-valueMin)*100:0;_set[this.orientation==="horizontal"?"left":"bottom"]=valPercent+"%";if(animate){this.handle.stop(1,1)["animate"](_set,o.animate);}else{this.handle.stop(1,1).css(_set);}
if(oRange==="min"&&this.orientation==="horizontal"){if(animate){this.range.stop(1,1)["animate"]({width:valPercent+"%"},o.animate);}else{this.range.stop(1,1).css({width:valPercent+"%"},o.animate);}}
if(oRange==="max"&&this.orientation==="horizontal"){if(animate){this.range["animate"]({width:(100-valPercent)+"%"},{queue:false,duration:o.animate});}else{this.range.css({width:(100-valPercent)+"%"},{queue:false,duration:o.animate});}}
if(oRange==="min"&&this.orientation==="vertical"){if(animate){this.range.stop(1,1)["animate"]({height:valPercent+"%"},o.animate);}else{this.range.stop(1,1).css({height:valPercent+"%"},o.animate);}}
if(oRange==="max"&&this.orientation==="vertical"){if(animate){this.range["animate"]({height:(100-valPercent)+"%"},{queue:false,duration:o.animate});}else{this.range.css({height:(100-valPercent)+"%"},{queue:false,duration:o.animate});}}}},_handleEvents:{keydown:function(event){var allowed,curVal,newVal,step,index=$(event.target).data("ui-slider-handle-index");switch(event.keyCode){case $.ui.keyCode.HOME:case $.ui.keyCode.END:case $.ui.keyCode.PAGE_UP:case $.ui.keyCode.PAGE_DOWN:case $.ui.keyCode.UP:case $.ui.keyCode.RIGHT:case $.ui.keyCode.DOWN:case $.ui.keyCode.LEFT:event.preventDefault();if(!this._keySliding){this._keySliding=true;$(event.target).addClass("ui-state-active");allowed=this._start(event,index);if(allowed===false){return;}}
break;}
